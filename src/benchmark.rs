...
/// Default benchmark iterations (can be overridden later via CLI if needed)
const DEFAULT_ITERATIONS: usize = 5_000_000;

/// Warmup iterations (to stabilize CPU/cache behavior)
...
/// Warmup phase to stabilize CPU frequency & cache
fn warmup(input: &str) {
...
/// Run all algorithms (single-thread)
...
where
    F: Fn(&str) -> String,
{
    // Warmup per algorithm (micro warmup)
    for _ in 0..10_000 {
        func(input);
    }
...
/// Output formatter (consistent + readable)
